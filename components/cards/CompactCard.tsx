// components/cards/CompactCard.tsx
...
export function CompactCard({
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    const isInteractive = target.closest('button, a, input, select, textarea, [role="button"], [role="option"]');
    if (!isInteractive) {
    ...
    >
      <CardHeader className="pb-3">
        <div className="space-y-3">
          ...
          {typeCircles && typeCircles.length > 0 && (
            <div className="flex items-center gap-1 min-w-0">
              {typeCircles.map((circle, index) => {
                if (circle.label === "—") {
                  return <div key={index} className={`${circle.color} shrink-0`} title="Разделитель" />;
                }
                if (circle.label === "mini-separator") {
                  return <div key={index} className={`${circle.color} shrink-0`} />;
                }

                const circleElement = (
                  <div
                    className={`h-5 px-1.5 rounded-full flex items-center justify-center text-xs text-white dark:text-white shrink-0 ${circle.color}`}
          ...
          {!isExpanded && actions && (
            <div className="flex items-center justify-end gap-1 min-w-0 pt-2">
              {actions}
            </div>
          )}
        </div>
