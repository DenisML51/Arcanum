...
import type { Biome } from "../hooks/useAlchemyStore";
...
export function CompactBiomeCard({
  ...
  const canAfford = playerGold >= biome.explorationCost;
  ...
  const totalChance = Math.min(100, (biome.availableIngredients || []).reduce((sum, ing) => sum + ing.chance, 0));
  ...
  const badges = [
    ...
    {
      label: `${biome.explorationCost} зм`,
  ...
  return (
    <CompactCard
      title={biome.name}
      subtitle={`${biome.explorationCost} зм`}
      badges={badges}
    ...
    >
      <div className="space-y-4">
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-3">
            {(!biome.availableIngredients || biome.availableIngredients.length === 0) ? (
              <p className="text-xs text-muted-foreground">Нет доступных ингредиентов</p>
            ) : (
              (biome.availableIngredients || []).map((available, index) => {
              const ingredient = ingredients.find(ing => ing.id === available.ingredientId);
              const rarityColors = {
        ...
        <div className="p-3 bg-blue-50/50 dark:bg-blue-950/50 rounded border border-blue-200 dark:border-blue-800">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-blue-700 dark:text-blue-300">
              Стоимость исследования: <strong>{biome.explorationCost} зм</strong> (оплачивается независимо от результата)
            </div>
          </div>
        </div>
      </div>
